...
public class IOTCAPIs {
	...
	* IOTC服务器无法找到指定的设备,可能造成的断开的设备还没有登录*/
	...
	* Please see IOTCConnectionMode for possible modes. */
	...
	//IOTCApis interface
	public native static int  IOTC_Get_Login_Info(int[] LoginInfo);//获取登陆信息，数组中的每一位代表某种连接的成功与否
	public native static void IOTC_Get_Version(int[] Version);//获取某个版本的IOTC，例如Version=0x01020304 时表示要获取1.2.3.4版本的IOTC
	public native static void IOTC_Set_Max_Session_Number(int num);//设置客户端连接设备的最大数量或设备连接客户端的最大数量
	/**
	 * 初始化IOTC 
	 * @param UDPPort  指定一个UDP端口。值为0时随机使用UDP端口
	 * @param P2PHostNamePrimary 指定的主要的主机名或IP地址的主人      不能为空
	 * @param P2PHostNameSecondary 指定的次要的主机名或IP地址的主人        不能为空
	 * @param P2PHostNameThird  可为空
	 * @param P2PHostNameFourth 可为空
	 * @return  初始化结果标志
	 */
	...
	public native static int  IOTC_Initialize2(int UDPPort);//与IOTC_Initialize的区别是先用IP连接不行再试主机名
	public native static int  IOTC_DeInitialize();
	public native static int  IOTC_Device_Login(String UID, String DeviceName, String DevicePWD);//设备连接到服务器
	public native static int  IOTC_Listen(int Timeout_ms);//设备监听客户端的连接状态，Timeout_ms为0是表示永远阻塞，当返回值<0时返回的为错误码，当返回值>0时返回值为连接session ID
	...
	public native static int  IOTC_Connect_ByUID(String UID);//客户端连接到指定UID的设备，成功返回会话ID否则返回错误码
	...
	public native static void IOTC_Connect_Stop();//客户端断开连接设备
	public native static int  IOTC_Get_SessionID();//客户端获取会话ID
	public native static int  IOTC_Connect_ByUID_Parallel(String UID, int SID);//客户端连接指定UID的设备和并绑定到指定的会话ID
	public native static int IOTC_Connect_Stop_BySID(int SID);

	public native static int  IOTC_Session_Check(int SID, St_SInfo s_Info);//设备或客户端获取指定SID（会话ID）的会话信息
	/**
	 * 客户端或设备通过指定的IOTC会话通道读取对方的数据信息
	 * @param SID 会话ID
	 * @param Buf 用来接收读取结果的字节数组缓冲区
	 * @param Max_size Buf的最大值
	 * @param Timeout_ms 该函数的最大执行时间，0表示立即返回
	 * @param ChID 通道ID
	 * @return 读取结果标志
	 */
	public native static int  IOTC_Session_Read(int SID, byte[] Buf, int Max_size,int Timeout_ms,int ChID);
	public native static int  IOTC_Session_Write(int SID, byte[] Buf, int Size, int ChID);//设备或客户端通过特定的IOTC会话通道向对方写（发送）数据
	public native static void IOTC_Session_Close(int SID);//客户端或设备关闭指定ID会话

	public native static int IOTC_Session_Get_Free_Channel(int SID);//客户端或设备为指定ID会话寻找通道
	public native static int IOTC_Session_Channel_ON(int SID, int ChID);//打开指定ID的会话通道
	public native static int IOTC_Session_Channel_OFF(int SID,int ChID);//关闭指定ID的会话通道
	public native static int IOTC_Get_Nat_Type();
	public native static int IOTC_IsDeviceSecureMode(int SID);
	public native static st_LanSearchInfo[]  IOTC_Lan_Search (int[] arrNum, int nWaitTimeMs);//客户端和设备同时在局域网环境中时,刻画段可以调用这个函数直接发现设备并连接它。
	public native static st_LanSearchInfo2[] IOTC_Lan_Search2(int[] arrNum, int nWaitTimeMs);

	public native static void IOTC_Set_Log_Path(String path, int maxSize);//设置日志文件的绝对路径
	public native static void IOTC_Listen_Exit();//设备关闭监听
	...
	public native static int  IOTC_Set_Partial_Encryption(int SID, int bPartialEncryption);//设置编码开关
	...
	public native static void IOTC_Setup_DetectNetwork_Timeout(int Timeout_ms);//设置检测网络的时间
